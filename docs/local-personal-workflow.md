...
**Status:** #7 and #8 backend PR A (#13) are merged. #8 UI PR B is implemented in this branch, pending review/merge; #9–#11 are not delivered. The contracts below do not imply released UI or verified generation quality. GitHub tracks current merge status.
...
| [#8 · Local takes](https://github.com/skaiy/h3.c-studio/issues/8) | A (#13) 已合并；B 在本分支实现，待评审/合并 | A：后端 take schema、旧数据迁移与选择 API；B：比较/采用 UI、下游连续性警告 | #7、A 已合并；B 直接到 main |
